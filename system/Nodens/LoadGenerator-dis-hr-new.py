...
cg_rate_list_all = [
    ...
    [0, 0, 0, 1], # 5
    [1.5,1,0.5,1], # lab1
    [1.5,0.75,0.75,1], # lab2
    [1.25,0.75,0.75,1.25], # lab3
    ]
cg_rate_list = []
...
if __name__ == "__main__":
    ...
    if args.head:  # 主节点只负责下发请求,不运行负载生成器
        ...
        for i in range(args.node_size):
            cmd=" ssh {} 'ulimit -n 100000;cd /state/partition/jcshi/CJY/opentelemetry-microservices-demo/system/Nodens;python3 LoadGenerator-dis-hr-new.py --head {} --qps {} --node_size {} -p {} -t {} -nodeName {} -d {} -host {}' "\
